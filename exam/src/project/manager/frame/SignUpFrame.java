...
public class SignUpFrame extends JFrame implements ActionListener {
	...
	public void init() {
		...
		lblNewLabel.setFont(new Font("나눔고딕코딩", Font.PLAIN, 25));
		...
		checkIdBtn.setFont(new Font("나눔고딕코딩", Font.BOLD, 10));
		...
		cancelBtn.setFont(new Font("나눔고딕코딩", Font.BOLD, 11));
		...
		joinBtn.setFont(new Font("나눔고딕코딩", Font.BOLD, 11));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (src == cancelBtn) {
			...
			if (idTxt.getText().length() == 0 || idTxt.getText().length() < 4 || idTxt.getText().length() > 10) {
				JOptionPane.showMessageDialog(this, "아이디는 4글자 이상 10글자 이하로 적어주세요!");
			} else if (!Pattern.matches("^[a-zA-Z][a-z0-9]+$", idTxt.getText())) {
				JOptionPane.showMessageDialog(this, "아이디는 영소문자 또는 숫자 조합으로 만들어주세요!");
			} else {
